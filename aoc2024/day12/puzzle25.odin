...
areaCost :: proc(data : []byte, width : int, height : int, startIndex : int, marked : ^[dynamic]bool, open : ^queue.Queue([2]int), fences : ^map[[3]int][3]int) -> int {
    ...
    for queue.len(open^) > 0 {
        ...
        neighbors := [4][2]int{ node - {0, 1}, node - {1, 0}, node + {0, 1}, node + {1, 0} }
        for neighbor, direction in neighbors {
            ...
            {
                startCorner := node + corners[direction]
                endCorner := node + corners[(direction + 1) % 4]
                fences[[3]int{ startCorner.x, startCorner.y, direction }] = { endCorner.x, endCorner.y, 1 }
                continue
            }
    ...
    for edgeStartAndDirection, &edgeEndAndFlag in fences {
        otherEdgeStartAndDirection := [3]int{ edgeEndAndFlag.x, edgeEndAndFlag.y, edgeStartAndDirection.z }
        if otherEdgeStartAndDirection in fences {
            otherEdgeEndAndFlag := &fences[otherEdgeStartAndDirection]
            edgeEndAndFlag.xy = otherEdgeEndAndFlag.xy
            otherEdgeEndAndFlag.z = 0
        }
    }
    perimeter := 0
    for edgeStartAndDirection, edgeEndAndFlag in fences {
        perimeter += edgeEndAndFlag.z 
    }
...
run :: proc() {
    ...
    for char, i in data {
        ...
        clear(&fences)
        result += areaCost(data[:], width, height, i, &marked, &open, &fences)
    }
