...
evaluate :: proc(test : int, numbers : []int, fromIndex : int, value : int) -> bool {
    ...
    tmp := numbers[fromIndex] > 0 ? math.log10(f32(numbers[fromIndex])) : 0
    numDigits := math.floor(tmp) + 1
    c := evaluate(test, numbers[:], fromIndex + 1, value * int(math.pow10(numDigits)) + numbers[fromIndex])
