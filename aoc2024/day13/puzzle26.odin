...
run :: proc() {
    ...
    for line in strings.split_lines_iterator(&it) {
        ...
        switch i {
        case 1:
            a.x, ok1 = strconv.parse_i64(parts[2][2 : len(parts[2]) - 1])
            a.y, ok2 = strconv.parse_i64(parts[3][2:])
            assert(ok1 && ok2)
        case 2:
            b.x, ok1 = strconv.parse_i64(parts[2][2 : len(parts[2]) - 1])
            b.y, ok2 = strconv.parse_i64(parts[3][2:])
            assert(ok1 && ok2)
        case 3:
            c.x, ok1 = strconv.parse_i64(parts[1][2 : len(parts[1]) - 1])
            c.y, ok2 = strconv.parse_i64(parts[2][2:])
            assert(ok1 && ok2)
