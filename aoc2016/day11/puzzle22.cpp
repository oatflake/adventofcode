...
const int8_t MAX_ARRAY_LENGTH = 7;
struct State {
    int8_t elevator;
...
bool operator==(const State& lhs, const State& rhs) {
    if (lhs.elevator != rhs.elevator)
        return false;
    int rhsColumns[MAX_ARRAY_LENGTH];
    ...
    for (int i = 0; i < MAX_ARRAY_LENGTH; i++) {
        ...
        for (int j = i; j < MAX_ARRAY_LENGTH; j++) {
            int c = rhsColumns[j];
            if (lhs.generators[i] == rhs.generators[c] && lhs.microchips[i] == rhs.microchips[c]) {
                rhsColumns[j] = rhsColumns[i];
                foundColumn = true;
                break;
            } 
        }
...
struct hash<State> {
    std::size_t operator()(State const& s) const {
        ...
        std::size_t hashes[MAX_ARRAY_LENGTH];
        for (int i = 0; i < MAX_ARRAY_LENGTH; i++) {
            std::size_t tmpRes = 0;
            hash_combine(tmpRes, s.generators[i]);
            hash_combine(tmpRes, s.microchips[i]);
            hashes[i] = tmpRes;
        }
        std::sort(std::begin(hashes), std::end(hashes));
        for (int i = 0; i < MAX_ARRAY_LENGTH; i++) {
            hash_combine(res, hashes[i]);
        }
        return res;
    }
};
...
std::tuple<State, StateInfo> readInput(std::vector<std::string> additionalItems) {
    std::unordered_map<std::string, int8_t> generators;
    std::unordered_map<std::string, int8_t> microchips;
    StateInfo info;
    for (const auto& name : additionalItems) {
        generators[name] = 0;
        microchips[name] = 0;
...
int main() {
    State state;
    StateInfo info;
    std::tie(state, info) = readInput({ "elerium", "dilithium" });
    std::string a;
