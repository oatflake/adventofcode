...
for i in range(finishLine):
    for reindeer in reindeers:
        reindeer.simulate()
    maxDistance = reduce(lambda a, b: a if a.distanceTraveled > b.distanceTraveled else b, reindeers).distanceTraveled
    for reindeer in reindeers:
        if reindeer.distanceTraveled == maxDistance:
            reindeer.points += 1

print reduce(lambda a, b: a if a.points > b.points else b, reindeers).points
